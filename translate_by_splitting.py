...
def translate_outside(text):
    # Split by curly braces, but we want to keep the braces to reassemble.
    # We'll use a regex to split by { or } and keep the delimiters.
    parts = re.split(r'({|})', text)
    # Now, even indices (0, 2, 4, ...) are outside, odd indices are inside (the braces themselves or the content?).
    # Actually, the split will return: [text_before_first_brace, '{', text_between_braces, '}', text_after_last_brace] if there is one pair.
    # But if there are multiple, it will be: [text0, '{', text1, '}', text2, '{', text3, '}', text4, ...]
    # So the outside parts are at indices 0, 2, 4, ... (even indices) and the inside parts are at odd indices.
    for i in range(len(parts)):
        if i % 2 == 0:  # outside part
            for eng, vie in translations.items():
                parts[i] = parts[i].replace(eng, vie)
    return ''.join(parts)
