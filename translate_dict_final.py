...
def translate_file(filepath):
    ...
    # We'll translate only outside of curly braces and strings.
    # We'll use a simple state machine: we iterate through the string and keep track of whether we are inside a string or inside curly braces.
    result = []
    i = 0
    n = len(content)
    in_string = False
    ...
    while i < n:
        ...
        if not in_string and ch == '{':
            ...
            if not in_string:
                ...
                # Check if it's escaped: we'll assume it's not escaped for simplicity, but we can check the previous character.
            ...
            # We'll accumulate until we hit a brace or quote.
            start = i
            while i < n and content[i] not in ('{', '}', '"', "'", "`") and not (content[i] == '\\' and i+1 < n and content[i+1] in ('"', "'", "`")):
                i += 1
